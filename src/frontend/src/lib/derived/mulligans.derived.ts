...
import type { MulligansScores, PrincipalId, MulligansHoleResult, Game } from "../../../../declarations/backend/backend.did";
...
export const currentHole = derived([mulligansScores, currentHoleStore], ([$mulligansScores, $currentHole]) => {
    return $currentHole || ($mulligansScores ? $mulligansScores.currentHole : 1);
});
...
export const availableMulligans = derived(
    ...
    ([$mulligansScores, $gameStateStore]) => {
      ...
      result[player1] = baseAwards + wins.player1 - $mulligansScores.golfer1MulligansUsed;
      result[player2] = baseAwards + wins.player2 - $mulligansScores.golfer2MulligansUsed;
...
export const mulligansHoleStates = derived(
    ...
    ([$gameState, $currentCourse]) => {
      ...
      const holeResults: MulligansHoleResult[] = ($gameState as any).holeResults || [];
      ...
      for (let i = 1; i <= totalHoles; i++) {
        ...
        states.push({ holeNumber: i, isComplete: isComplete, isLocked: isLocked });
      }
      return states;
    }
  );
