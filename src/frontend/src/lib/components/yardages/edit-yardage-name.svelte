<script lang="ts">
    ...
    export let yardageSetName: string;
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 sm:p-0">
        <div class="w-full sm:w-[90vw] md:w-[60vw] lg:w-1/3 p-4 sm:p-8 bg-white rounded-lg">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-xl text-black sm:text-2xl md:text-3xl condensed">EDIT YARDAGE NAME</h2>
                <button 
            ...
            <p class="mb-6 text-sm text-BrandDarkGray">Give it a good one! </p>
            <div>
                <label for="yardageSetName" class="block mb-2 text-sm text-black">NAME</label>
                <input
                  id="yardageSetName"
                  type="text"
                  class="w-full p-2 text-black rounded sm:p-3 bg-BrandLightGray"
                  placeholder="Enter"
