import { c as $r, g as fu, _ as lu } from "./BcYFQVpD.js";
const qh = !0,
  Vh = !1;
function Ko(t) {
  var e,
 ...
 */ (function (t) {
  ...
  function A(d) {
    const u = d.length < 0 ? 0 : L(d.length) | 0,
      f = l(u);
  ...
  function j(d) {
    ...
    let f = "",
      v = 0;
    for (; v < u; )
      f += String.fromCharCode.apply(String, d.slice(v, (v += W)));
    return f;
  }
  function G(d, u, f) {
    let v = "";
...
function Yo(t) {
  return typeof t == "string" && (t = Pu(t)), Lo(t), t;
}
class ku {
  clone() {
...
const xn = "__principal__",
  ...
  Wu = (t) => t.reduce((e, r) => e + r.toString(16).padStart(2, "0"), "");
let Ae = class Pn {
  constructor(e) {
  ...
  toHex() {
    return Wu(this._arr).toUpperCase();
  }
  toText() {
...
const Lu = new RegExp(/^[0-9a-fA-F]+$/);
function Gt(t) {
  if (!Lu.test(t)) throw new Error("Invalid hexadecimal string.");
  const e = [...t]
...
function la(t, e) {
  ...
  function o(A) {
    ...
    const C = [],
      L = Number(Ze(A));
    for (let z = 0; z < L; z++) C.push(Number(Zt(A)));
    return [w, C];
  }
  const [l, a] = o(r);
 ...
 */ (function (t) {
  ...
  function A(h) {
    for (
      var s = h.length < 0 ? 0 : L(h.length) | 0, c = l(s), y = 0;
      y < s;
      y += 1
    )
      c[y] = h[y] & 255;
  ...
  (function (e) {
    ...
    function w(Z) {
      ...
      return (
        ...
        (X.isGreaterThanOrEqualTo = X.gte =
          function (b, x) {
            return (x = L(this, new m(b, x))) === 1 || x === 0;
          }),
          ...
          return (
            O === null
              ? k
                ...
                : (x = "NaN")
              : (b == null
                  ? (x = O <= U || O >= W ? re(C(B.c), O) : P(C(B.c), O, "0"))
                  : b === 10 && ee
    ...
    function L(Z, Y) {
      ...
      if (((J = X && !X[0]), (Q = he && !he[0]), J || Q))
        return J ? (Q ? 0 : -N) : le;
      if (le != N) return le;
      if (((J = le < 0), (Q = U == W), !X || !he))
        return Q ? 0 : !X ^ J ? 1 : -1;
      if (!Q) return (U > W) ^ J ? 1 : -1;
      for (N = (U = X.length) < (W = he.length) ? U : W, le = 0; le < N; le++)
        if (X[le] != he[le]) return (X[le] > he[le]) ^ J ? 1 : -1;
      return U == W ? 0 : (U > W) ^ J ? 1 : -1;
    }
    function z(Z, Y, J, Q) {
      if (Z < Y || Z > J || Z !== o(Z))
        throw Error(
...
const { URLWithLegacySupport: zh, format: Wh } = Ha,
  {
...
const { Buffer: Rt } = wn,
  ...
  eo = Le.SYMS,
  Br = Le.TAG,
  mf = (Le.MT.SIMPLE_FLOAT << 5) | Le.NUMBYTES.TWO,
  bf = (Le.MT.SIMPLE_FLOAT << 5) | Le.NUMBYTES.FOUR,
  _f = (Le.MT.SIMPLE_FLOAT << 5) | Le.NUMBYTES.EIGHT,
  xf = (Le.MT.SIMPLE_FLOAT << 5) | Le.SIMPLE.TRUE,
  Ef = (Le.MT.SIMPLE_FLOAT << 5) | Le.SIMPLE.FALSE,
  vf = (Le.MT.SIMPLE_FLOAT << 5) | Le.SIMPLE.UNDEFINED,
  to = (Le.MT.SIMPLE_FLOAT << 5) | Le.SIMPLE.NULL,
...
function Mn(t) {
  if (t instanceof Wa.Tagged) return Mn(t.value);
  if (typeof t == "string") return La(t);
...
Hr.SelfDescribeCborSerializer = Wf;
(function (t) {
  function e(i) {
    ...
    wNAF(r, n, i) {
      ...
      for (let I = 0; I < o; I++) {
        ...
        const $ = A,
          ...
          L = I % 2 !== 0,
          z = w < 0;
        w === 0 ? (p = p.add(Hi(L, n[$]))) : (a = a.add(Hi(z, n[C])));
      }
      return { p: a, f: p };
    },
...
function yo(t) {
  ...
  function l($) {
    const { a: C, b: L } = e,
      z = r.sqr($),
      q = r.mul(z, $);
    return r.add(r.add(q, r.mul($, C)), L);
  }
  ...
  function p($) {
    ...
    let re;
    try {
      re = typeof $ == "bigint" ? $ : xt(at("private key", $, L));
    } catch {
      throw new Error(
        "invalid private key, expected hex or " + L + " bytes, got " + typeof $,
      );
    }
  ...
  const E = cn(($, C) => {
      const { px: L, py: z, pz: q } = $;
      if (r.eql(q, r.ONE)) return { x: L, y: z };
      const re = $.is0();
  ...
  class I {
    ...
    static normalizeZ(C) {
      const L = r.invertBatch(C.map((z) => z.pz));
      return C.map((z, q) => z.toAffine(L[q])).map(I.fromAffine);
    }
    static fromHex(C) {
      const L = I.fromAffine(o(at("pointHex", C)));
      return L.assertValidity(), L;
    }
    ...
    equals(C) {
      g(C);
      const { px: L, py: z, pz: q } = this,
        { px: re, py: P, pz: Z } = C,
        Y = r.eql(r.mul(L, Z), r.mul(re, q)),
    ...
    multiplyUnsafe(C) {
      ...
      if (!L || w.hasPrecomputes(this))
        return w.wNAFCachedUnsafe(this, C, I.normalizeZ);
      let { k1neg: re, k1: P, k2neg: Z, k2: Y } = L.splitScalar(C),
        J = q,
        Q = q,
        X = this;
      for (; P > yr || Y > yr; )
      ...
      return (
        re && (J = J.negate()),
        Z && (Q = Q.negate()),
        (Q = new I(r.mul(Q.px, L.beta), Q.py, Q.pz)),
        J.add(Q)
      );
    }
    multiply(C) {
    ...
    multiplyAndAddUnsafe(C, L, z) {
      const q = I.BASE,
        re = (Z, Y) =>
          Y === yr || Y === Ge || !Z.equals(q)
            ? Z.multiplyUnsafe(Y)
            : Z.multiply(Y),
        P = re(this, L).add(re(C, z));
    ...
    clearCofactor() {
      ...
      return C === Ge ? this : L ? L(I, this) : this.multiplyUnsafe(e.h);
    }
    toRawBytes(C = !0) {
...
function _l(t, e) {
  const r = t.ORDER;
  let n = yr;
  for (let w = r - Ge; w % Wt === yr; w /= Wt) n += Ge;
  const i = n,
    o = Wt << (i - Ge - Ge),
    l = o * Wt,
    a = (r - Ge) / l,
  ...
  let A = (w, $) => {
    ...
    z = t.mul(z, $);
    let q = t.mul(w, z);
    (q = t.pow(q, p)), (q = t.mul(q, L)), (L = t.mul(q, $)), (z = t.mul(q, w));
    let re = t.mul(z, L);
    q = t.pow(re, E);
    let P = t.eql(q, t.ONE);
    (L = t.mul(z, I)),
...
function Sl(t) {
  const { ORDER: e } = t,
    ...
    i = ll(e),
    o = r.div(r.ONE, Ct),
    l = Li(r, n, r.ORDER, 2)[0],
    a = ({ c0: N, c1: U }, { c0: W, c1: j }) => ({
      c0: r.add(N, W),
      c1: r.add(U, j),
    }),
    p = ({ c0: N, c1: U }, { c0: W, c1: j }) => ({
      c0: r.sub(N, W),
      c1: r.sub(U, j),
    }),
    g = ({ c0: N, c1: U }, W) => {
      if (typeof W == "bigint") return { c0: r.mul(N, W), c1: r.mul(U, W) };
      const { c0: j, c1: G } = W;
      let D = r.mul(N, j),
        ie = r.mul(U, G);
      const ge = r.sub(D, ie),
    ...
    E = ({ c0: N, c1: U }) => {
      const W = r.add(N, U),
        j = r.sub(N, U),
        G = r.add(N, N);
      return { c0: r.mul(W, j), c1: r.mul(G, U) };
    },
    S = (N) => {
      if (N.length !== 2) throw new Error("invalid tuple");
      const U = N.map((W) => r.create(W));
    ...
    w = {
      ...
      eql: ({ c0: N, c1: U }, { c0: W, c1: j }) => r.eql(N, W) && r.eql(U, j),
      ...
      inv: ({ c0: N, c1: U }) => {
        const W = r.inv(r.create(N * N + U * U));
        return { c0: r.mul(W, r.create(N)), c1: r.mul(W, r.create(-U)) };
      },
      sqrt: (N) => {
        ...
        const j = r.sqrt(r.sub(r.sqr(U), r.mul(r.sqr(W), n)));
        let G = r.mul(r.add(j, U), o);
        const D = i(r, G);
        !r.is0(D) && !r.eql(D, r.ONE) && (G = r.sub(G, j));
        const ie = r.sqrt(G),
          ge = w.create({ c0: ie, c1: r.div(r.mul(W, o), ie) });
      ...
      mulByB: t.Fp2mulByB,
      fromBigTuple: S,
      frobeniusMap: ({ c0: N, c1: U }, W) => ({
        c0: N,
        c1: r.mul(U, l[W % 2]),
      }),
    },
    $ = ({ c0: N, c1: U, c2: W }, { c0: j, c1: G, c2: D }) => ({
      c0: w.add(N, j),
      c1: w.add(U, G),
      c2: w.add(W, D),
    }),
    C = ({ c0: N, c1: U, c2: W }, { c0: j, c1: G, c2: D }) => ({
      c0: w.sub(N, j),
      c1: w.sub(U, G),
      c2: w.sub(W, D),
    }),
    L = ({ c0: N, c1: U, c2: W }, j) => {
      if (typeof j == "bigint")
        return { c0: w.mul(N, j), c1: w.mul(U, j), c2: w.mul(W, j) };
      ...
      return {
        c0: w.add(
          ge,
          w.mulByNonresidue(
            w.sub(w.mul(w.add(U, W), w.add(D, ie)), w.add(oe, fe)),
          ),
        ),
        c1: w.add(
        ...
        c2: w.sub(w.add(oe, w.mul(w.add(N, W), w.add(G, ie))), w.add(ge, fe)),
      };
    },
    z = ({ c0: N, c1: U, c2: W }) => {
      let j = w.sqr(N),
        G = w.mul(w.mul(N, U), Ct),
        D = w.mul(w.mul(U, W), Ct),
        ie = w.sqr(W);
      return {
        c0: w.add(w.mulByNonresidue(D), j),
        c1: w.add(w.mulByNonresidue(ie), G),
        c2: w.sub(
          w.sub(w.add(w.add(G, w.sqr(w.add(w.sub(N, U), W))), D), j),
          ie,
        ),
      };
    },
    [q, re] = Li(w, A, r.ORDER, 6, 2, 3),
    P = {
      ORDER: w.ORDER,
      isLE: w.isLE,
      BITS: 3 * w.BITS,
      ...
      create: (N) => N,
      isValid: ({ c0: N, c1: U, c2: W }) =>
        w.isValid(N) && w.isValid(U) && w.isValid(W),
      is0: ({ c0: N, c1: U, c2: W }) => w.is0(N) && w.is0(U) && w.is0(W),
      neg: ({ c0: N, c1: U, c2: W }) => ({
        c0: w.neg(N),
        c1: w.neg(U),
        c2: w.neg(W),
      }),
      eql: ({ c0: N, c1: U, c2: W }, { c0: j, c1: G, c2: D }) =>
        w.eql(N, j) && w.eql(U, G) && w.eql(W, D),
      sqrt: oo,
      div: (N, U) =>
      ...
      sqrN: z,
      inv: ({ c0: N, c1: U, c2: W }) => {
        let j = w.sub(w.sqr(N), w.mulByNonresidue(w.mul(W, U))),
          G = w.sub(w.mulByNonresidue(w.sqr(W)), w.mul(N, U)),
          D = w.sub(w.sqr(U), w.mul(N, W)),
          ie = w.inv(
            w.add(
              w.mulByNonresidue(w.add(w.mul(W, G), w.mul(U, D))),
              w.mul(N, j),
            ),
          );
      ...
      toBytes: ({ c0: N, c1: U, c2: W }) =>
        Ke(w.toBytes(N), w.toBytes(U), w.toBytes(W)),
      cmov: ({ c0: N, c1: U, c2: W }, { c0: j, c1: G, c2: D }, ie) => ({
        c0: w.cmov(N, j, ie),
        c1: w.cmov(U, G, ie),
        c2: w.cmov(W, D, ie),
      }),
      ...
      frobeniusMap: ({ c0: N, c1: U, c2: W }, j) => ({
        c0: w.frobeniusMap(N, j),
        c1: w.mul(w.frobeniusMap(U, j), q[j % 6]),
        c2: w.mul(w.frobeniusMap(W, j), re[j % 6]),
      }),
      mulByFp2: ({ c0: N, c1: U, c2: W }, j) => ({
        c0: w.mul(N, j),
        c1: w.mul(U, j),
        c2: w.mul(W, j),
      }),
      mulByNonresidue: ({ c0: N, c1: U, c2: W }) => ({
        c0: w.mulByNonresidue(W),
        c1: N,
        c2: U,
      }),
      mul1: ({ c0: N, c1: U, c2: W }, j) => ({
        c0: w.mulByNonresidue(w.mul(W, j)),
        c1: w.mul(N, j),
        c2: w.mul(U, j),
      }),
      mul01({ c0: N, c1: U, c2: W }, j, G) {
        let D = w.mul(N, j),
          ie = w.mul(U, G);
        return {
          c0: w.add(w.mulByNonresidue(w.sub(w.mul(w.add(U, W), G), ie)), D),
          c1: w.sub(w.sub(w.mul(w.add(j, G), w.add(N, U)), D), ie),
          c2: w.add(w.sub(w.mul(w.add(N, W), j), D), ie),
        };
      },
    },
    Z = Li(w, A, r.ORDER, 12, 1, 6)[0],
    Y = ({ c0: N, c1: U }, { c0: W, c1: j }) => ({
      c0: P.add(N, W),
      c1: P.add(U, j),
    }),
    J = ({ c0: N, c1: U }, { c0: W, c1: j }) => ({
      c0: P.sub(N, W),
      c1: P.sub(U, j),
    }),
    Q = ({ c0: N, c1: U }, W) => {
      if (typeof W == "bigint") return { c0: P.mul(N, W), c1: P.mul(U, W) };
      let { c0: j, c1: G } = W,
        D = P.mul(N, j),
        ie = P.mul(U, G);
      return {
    ...
    X = ({ c0: N, c1: U }) => {
      ...
      return {
        c0: P.sub(
          P.sub(P.mul(P.add(P.mulByNonresidue(U), N), P.add(N, U)), W),
          P.mulByNonresidue(W),
        ),
        c1: P.add(W, W),
      };
  ...
  function he(N, U) {
    const W = w.sqr(N),
      j = w.sqr(U);
    return {
      first: w.add(w.mulByNonresidue(j), W),
      second: w.sub(w.sub(w.sqr(w.add(N, U)), W), j),
    };
  }
  const le = {
    ORDER: w.ORDER,
    ...
    eql: ({ c0: N, c1: U }, { c0: W, c1: j }) => P.eql(N, W) && P.eql(U, j),
    sqrt: oo,
    inv: ({ c0: N, c1: U }) => {
      let W = P.inv(P.sub(P.sqr(N), P.mulByNonresidue(P.sqr(U))));
      return { c0: P.mul(N, W), c1: P.neg(P.mul(U, W)) };
    },
    div: (N, U) =>
    ...
    frobeniusMap(N, U) {
      const { c0: W, c1: j, c2: G } = P.frobeniusMap(N.c1, U),
        D = Z[U % 12];
      return {
        c0: P.frobeniusMap(N.c0, U),
        c1: P.create({ c0: w.mul(W, D), c1: w.mul(j, D), c2: w.mul(G, D) }),
      };
    },
    mulByFp2: ({ c0: N, c1: U }, W) => ({
      c0: P.mulByFp2(N, W),
      c1: P.mulByFp2(U, W),
    }),
    conjugate: ({ c0: N, c1: U }) => ({ c0: N, c1: P.neg(U) }),
    mul014: ({ c0: N, c1: U }, W, j, G) => {
      let D = P.mul01(N, W, j),
        ie = P.mul1(U, G);
      return {
        c0: P.add(P.mulByNonresidue(ie), D),
        c1: P.sub(P.sub(P.mul01(P.add(U, N), W, w.add(j, G)), D), ie),
      };
    },
    mul034: ({ c0: N, c1: U }, W, j, G) => {
      const D = P.create({
          c0: w.mul(N.c0, W),
          c1: w.mul(N.c1, W),
          c2: w.mul(N.c2, W),
        ...
        ie = P.mul01(U, j, G),
        ge = P.mul01(P.add(N, U), w.add(W, j), G);
      return {
        c0: P.add(P.mulByNonresidue(ie), D),
            ...
            const I = await gr.create({
              ...
              A = (C, L) => {
                if (L === "subnet") {
                  ...
                  return { path: L, data: z };
                } else return { path: L, data: pt(C.lookup(bs(L, n))) };
              },
              { path: w, data: $ } = A(I, i[g]);
            if (!$)
              console.warn(
...
class h0 extends Do {
  ...
  process(e, r) {
    for (let q = 0; q < 16; q++, r += 4)
      (Lt[q] = e.getUint32(r)), (Yt[q] = e.getUint32((r += 4)));
    for (let q = 16; q < 80; q++) {
      const re = Lt[q - 15] | 0,
        P = Yt[q - 15] | 0,
        ...
        N = ve.add4H(le, Z, X, Lt[q - 7], Lt[q - 16]);
      (Lt[q] = N | 0), (Yt[q] = le | 0);
    }
    let {
      Ah: n,
    ...
    for (let q = 0; q < 80; q++) {
      ...
      (L = $ | 0),
        ...
        (o = n | 0),
        (l = i | 0);
      const W = ve.add3L(X, le, U);
      (n = ve.add3H(W, Q, he, N)), (i = W | 0);
    }
    ({ h: n, l: i } = ve.add(this.Ah | 0, this.Al | 0, n | 0, i | 0)),
      ...
      ({ h: L, l: z } = ve.add(this.Hh | 0, this.Hl | 0, L | 0, z | 0)),
      this.set(n, i, o, l, a, p, g, E, S, I, A, w, $, C, L, z);
  }
  roundClean() {
    Lt.fill(0), Yt.fill(0);
  }
  ...
  incrementCurrentInterval() {
    ...
    return (
      Tt(
        this,
        Nt,
        Math.min(et(this, Nt, "f") * et(this, Wn, "f"), et(this, Ln, "f")),
        "f",
      ),
    ...
    rt.add(this),
      ...
      Qn.set(this, (l, a) => {
        ...
        for (const I of g) {
          ...
          if (Tr.verify(I.signature, new Uint8Array(L), new Uint8Array(q)))
            return l;
...
`);
      ...
      const L = await this.parseTimeFromResponse(C);
      return (
        L > 0 &&
          (this.log.print("Read state response time:", L),
          Je(this, Jr, L, "f")),
        C
      );
    } catch ($) {
  ...
  r.annotations.includes("query") || r.annotations.includes("composite_query")
    ...
    : (i = async (l, ...a) => {
        ...
        if (z.body && z.body.certificate) {
          ...
          const X = [new TextEncoder().encode("request_status"), L];
...
class D0 {
  ...
  get rawKey() {
    return Wr(this, Dt, "f").rawKey;
  }
  get derKey() {
    return Wr(this, Dt, "f").derKey;
  }
  toDer() {
    return Wr(this, Dt, "f").toDer();
  }
  getPublicKey() {
    return Wr(this, Dt, "f");
  }
  getPrincipal() {
    return Ae.from(Wr(this, Dt, "f").rawKey);
  }
  transformRequest() {
...
let As = {
  get(t, e, r) {
    if (t instanceof IDBTransaction) {
      ...
      if (e === "objectStoreNames") return t.objectStoreNames || Wc.get(t);
      if (e === "store")
...
function fh(t) {
  return t === IDBDatabase.prototype.transaction &&
    ...
    ? function (e, ...r) {
        ...
        return Wc.set(n, e.sort ? e.sort() : [e]), or(n);
      }
    : oh().includes(t)
      ? function (...e) {
...
const Lc = "auth-client-db",
  Yc = "ic-keyval",
  yh = async (t = Lc, e = Yc, r) => (
    Zc &&
      localStorage != null &&
...
class Ws {
  ...
  static async create(e) {
    const { dbName: r = Lc, storeName: n = Yc, version: i = Eh } = e ?? {},
      o = await yh(r, n, i);
    return new Ws(o, n);
  }
  async set(e, r) {
